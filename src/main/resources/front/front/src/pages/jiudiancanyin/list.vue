...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>餐饮名称</div>
        <el-input v-model="formSearch.canyinmingcheng" placeholder="餐饮名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>餐饮分类</div>
        <el-input v-model="formSearch.canyinfenlei" placeholder="餐饮分类" clearable></el-input>
      </el-form-item>
	...
	<div class="list" :style='{"width":"100%","margin":"0 0 10px","background":"none","order":"3"}'>
		...
		<div class="list1 index-pv1" :style='{"border":"1px solid #dfdfdf","padding":"20px 10px","margin":"16px 0 0 0","borderRadius":"16px","flexWrap":"wrap","background":"#fff","display":"flex","width":"1200px","height":"auto"}'>
			<div :style='{"margin":"0 1% 20px 1%","borderRadius":"8px","background":"none","display":"inline-block","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div :style='{"padding":"0 10px","boxShadow":"1px 1px 1px #ddd","margin":"4px 0 0 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"8px","background":"radial-gradient(circle, rgba(250,250,250,1) 0%, rgba(220,230,250,0.5) 100%)","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.canyinmingcheng}}</div>
				<div :style='{"padding":"0 10px","boxShadow":"1px 1px 1px #ddd","margin":"4px 0 0 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"8px","background":"radial-gradient(circle, rgba(250,250,250,1) 0%, rgba(220,230,250,0.5) 100%)","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">价格:{{item.jiage}}</div>
				<div :style='{"padding":"0 10px","boxShadow":"1px 1px 1px #ddd","margin":"4px 0 0 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"8px","background":"radial-gradient(circle, rgba(250,250,250,1) 0%, rgba(220,230,250,0.5) 100%)","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">数量:{{item.shuliang}}</div>
			</div>
		</div>
	...
	<div class="hot" :style='{"width":"100%","margin":"20px 0 0 0","borderRadius":"8px 8px 0 0","background":"none","height":"auto","order":"5"}'>
	  ...
	  <div :style='{"border":"1px solid #dfdfdf","padding":"20px","boxShadow":"1px 2px 3px #eee,inset 0px 16px 16px 0px #eee","borderRadius":"0 0 8px 8px","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
	    <div v-for="item in hotList" :key="item" :style='{"width":"23%","background":"#fff","height":"auto"}' @click="toDetail(item)">
	      ...
	      <div :style='{"cursor":"pointer","padding":"0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#666","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis"}'>{{item.canyinmingcheng}}</div>
	      <div :style='{"cursor":"pointer","padding":"0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#666","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis"}'>{{item.jiage}}</div>
	      <div :style='{"cursor":"pointer","padding":"0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#666","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis"}'>{{item.shuliang}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.canyinmingcheng != '') searchWhere.canyinmingcheng = '%' + this.formSearch.canyinmingcheng + '%';
        if (this.formSearch.canyinfenlei != '') searchWhere.canyinfenlei = '%' + this.formSearch.canyinfenlei + '%';
        this.$http.get('jiudiancanyin/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/jiudiancanyinDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
